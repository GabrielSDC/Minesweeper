...
function setupGame() {
    ...
    const mode = select.selectedIndex;
    
    game = new Field(dificulty[mode]);
    game.state = "playing";
...
function finish(end) {
    ...
    for (let cell of game.mines) {
        showMines(cell.x, cell.y, newClass);
    }
...
// show where all the mines are when the game is lost
...
function cleanAround(cell) {
    for (let edge of cell.edges) {
        if (edge && edge.state === "none") {
            pressCell(edge.x, edge.y);
        }
    }
}

// checks if the number of flagged cells is the same as the number of mines around
function checkFlags(cell) {
    let flags = 0;

    for (let edge of cell.edges) {
        if (edge && edge.state === "flagged")
            flags++;
    }
